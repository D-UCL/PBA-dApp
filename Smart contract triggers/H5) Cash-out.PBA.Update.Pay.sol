...
contract trigger is owned {
  ...
  function invoke(address _recordId,IScheduleIII.Data memory ScheduleIII_Data) public payable returns(bool){
    ...
    require(msg.value == ScheduleIII_GetById_record.A_Actual);
          payable(ScheduleIII_GetById_record.A_Payee).transfer(msg.value);
    ...
    ScheduleIII_Data.A_Status = 'Paid';
  ...
  function Condition0(address _msgSenderBase) private  {
        ...
        for(uint x = 0; x < PBA.GetLength(); x++){
          ...
          (PBA_GetByIndex_recordId,PBA_GetByIndex_record) = PBA.GetByIndex(x);
            ...
            if(_msgSenderBase == PBA_GetByIndex_record.A_Address){
              contains = true;
              break;
            }
