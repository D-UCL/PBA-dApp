...
contract trigger is owned {
  ...
  function invoke(address _recordId,ICashoutII.Data memory CashoutII_Data) public payable returns(bool){
    ...
    require(msg.value == CashoutII_GetById_record.A_Actual);
          payable(CashoutII_GetById_record.A_Payee).transfer(msg.value);
  ...
  function Condition0(address _msgSenderBase) private  {
        ...
        for(uint x = 0; x < PBAmanager.GetLength(); x++){

          address PBAmanager_GetByIndex_recordId;
          IPBAmanager.Data memory PBAmanager_GetByIndex_record;
          (PBAmanager_GetByIndex_recordId,PBAmanager_GetByIndex_record) = PBAmanager.GetByIndex(x);

            if(_msgSenderBase == PBAmanager_GetByIndex_record.A_Wallet){
              contains = true;
              break;
            }
